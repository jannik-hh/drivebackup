...
public class DefaultGDirectory implements GDirectory {
  ...
  private DefaultGDirectory(Drive drive, String parentID) {
    ...
    try {
      this.files =
          QueryExecutorWithRetry.executeWithRetry(new GetFilesOfDirectoryCall(parentID, drive));
    } catch (IOException e) {
  ...
  private Optional<GDirectory> findDirectory(String name) throws IOException {
    ...
    Optional<File> dir =
        files
            ...
            .filter(
                (file) ->
                    FOLDER_MIME_TYPE.equals(file.getMimeType()) && name.equals(file.getTitle()))
            .findFirst();
  ...
  private GDirectory createDirectory(String name) throws IOException {
    File newDir =
        QueryExecutorWithRetry.executeWithRetry(new CreateDirectoryCall(name, parentID, drive));
    files.add(newDir);
  ...
  private Optional<File> findFile(String fileName) throws IOException {
    ...
    return files
        ...
        .filter(
            (aFile) ->
                !FOLDER_MIME_TYPE.equals(aFile.getMimeType()) && fileName.equals(aFile.getTitle()))
        .findFirst();
  }
