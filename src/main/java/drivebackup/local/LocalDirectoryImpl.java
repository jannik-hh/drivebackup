...
public class LocalDirectoryImpl implements LocalDirectory {
  ...
  public LocalDirectoryImpl(
      ...
      Predicate<File> fileIgnoredPredicate) {
  ...
  public Stream<LocalFile> getFiles() throws IOException {
    return getChildren()
        ...
        .map((file) -> new LocalFileImpl(file, encryptionService, stringEncryptionService));
  }
        ...
        .map(
            (dir) ->
                new LocalDirectoryImpl(
                    dir, encryptionService, stringEncryptionService, fileIgnoredPredicate));
  }

  protected Stream<File> getChildren() throws IOException {
    return Files.list(directory.toPath())
        .map((path) -> path.toFile())
        .filter(fileIgnoredPredicate.negate());
  }
}
