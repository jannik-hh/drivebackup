...
public class CopyFileCallTest extends BaseGDirTest {
	...
	public void test() throws IOException{
		...
		File copiedFile = new CopyFileCall(originFile, targetDir.getID(), "text2.txt", googleDrive).call();
		assertNotNull(copiedFile);
		assertEquals(originFile.getMd5Checksum(), copiedFile.getMd5Checksum());
		assertEquals(new OriginMD5ChecksumAccessor(originFile).get(), new OriginMD5ChecksumAccessor(copiedFile).get());
		assertThat(copiedFile.getParents().stream().map((ref) -> ref.getId()).collect(Collectors.toList()), contains(targetDir.getID()));
	}
}
